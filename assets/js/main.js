...
function renderPage() {
	...
	calenderContainer.addEventListener('touchmove', function (event) {
		if (!tearInProgress) return;
	
		let touchY = event.touches[0].clientY;
		let deltaY = touchY - startTouchY;
		let tearAmount = deltaY * 0.2; // Adjust the multiplier for tear effect sensitivity
	
		let newPageY = startPageY + deltaY;
		if (newPageY > startPageY) {
			// Prevent dragging the page below its initial position
			event.preventDefault();
		}
	
		calenderContainer.style.transform = `translateY(${tearAmount}px)`;
	});
	
	calenderContainer.addEventListener('touchend', function () {
		if (!tearInProgress) return;
	
		if (Math.abs(startTouchY - event.changedTouches[0].clientY) < 10) {
			// If the touch was light, reset the page position without tearing
			calenderContainer.style.transition = 'transform 0.3s ease';
			calenderContainer.style.transform = 'translateY(0)';
		} else {
			// Perform tear animation
			calenderContainer.style.transition = 'transform 0.5s cubic-bezier(0.6, 0.2, 0.1, 1)';
			calenderContainer.style.transform = 'translateY(200vh)'; // Adjust the distance for tear animation
